/*
BattleShip Game
Design and implement a battleship game to be played between two players until one comes out as
the winner.
Requirements:
    ...
    Square shape. Both the players should be assigned equal fleet.
    ...
    (0, 4), (4, 0) and (4,4)
    ● Ships will remain stationary. No two ships should overlap with each other. However they can
    touch boundaries with each other.
...
Mandatory:
    1. initGame() : This will initialize the game with a battlefield of size NxN. Where the left half of N/2xN will be
                    assigned to PlayerA and the right half will be assigned to PlayerB
    2. addShip() : This should add a ship of given size at the given coordinates in both the player’s fleet.
    3. startGame() : This will begin the game, where PlayerA will always take the first turn. The output of each
...
Optional:
    1. viewBattleField(): This will display the battlefield as a NxN grid and all the ships along with the grids occupied
                          by each ship. PlayerA’s ship with id SH1 will be marked as A-SH1, with id SH2 as A-SH2 and
                          so on. Whereas PlayerB’s ships will be marked as B-SH1, B-SH2 and so on.
